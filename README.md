...
해외 기술 콘텐츠를 자동 수집하고, AI로 요약/번역한 뒤, 한국어 블로그 포스트로 멀티 플랫폼 발행하는 파이프라인.

```
HN / GitHub / RSS / YouTube
        ↓ 수집 + 중복제거 + 스코어링
  OpenAI GPT-4o-mini 요약 · 번역  (Gemini / Claude fallback)
        ↓ 페르소나 스타일링
Ghost / WordPress / 네이버 블로그 / 티스토리 / Telegram / Discord
```

## 주요 기능

- **4개 소스 자동 수집** — Hacker News, GitHub Trending, RSS/Atom, YouTube
- **멀티 AI 프로바이더** — OpenAI GPT-4o-mini (메인) + Gemini Flash + Claude Haiku 자동 fallback
- **페르소나 스타일링** — YAML로 정의한 블로거 톤/스타일로 자연스러운 한국어 포스트 생성
- **7채널 동시 발행** — Ghost + WordPress + 네이버 블로그 + 티스토리 + Telegram + Discord + SNS
...
cp .env.example .env
# .env 편집: OPENAI_API_KEY, GHOST_ADMIN_KEY, DASHBOARD_PASSWORD 입력
...
```bash
OPENAI_API_KEY=sk-xxxx           # AI 요약/번역 (필수)
GHOST_ADMIN_KEY=id:secret        # Ghost 발행 (localhost:2368/ghost 에서 발급)
DASHBOARD_PASSWORD=changeme      # 대시보드 로그인
```

→ HN + RSS만으로 수집 시작. GitHub/YouTube는 각 API 키 추가 시 활성화.
→ Gemini/Claude 키 추가 시 자동 fallback 체인 활성화.
...
├── collectors/          # HN, GitHub, RSS, YouTube
├── processing/          # 중복제거, 스코어링, AI 요약, 페르소나 스타일링
├── publishers/          # Ghost, WordPress, 네이버, 티스토리, Telegram, Discord, SNS
├── orchestrator/        # 파이프라인 오케스트레이터 + APScheduler
├── ai/                  # 멀티 AI 클라이언트 (OpenAI/Gemini/Claude fallback)
...
```
[4개 소스] → RawItem → DedupService → ScoringEngine → top N
                                                        ↓
                                          Summarizer (OpenAI → Gemini → Claude)
                                                        ↓
                                           Humanizer (OpenAI → Gemini → Claude)
                                                        ↓
                          Ghost / WordPress / Naver / Tistory / TG / Discord
```
...
| AI | OpenAI GPT-4o-mini (메인) + Gemini Flash + Claude Haiku (fallback) |
| CMS | Ghost 5, WordPress (REST API) |
| 블로그 | 네이버 블로그, 티스토리 (Playwright 자동화) |
| DB | PostgreSQL 15 |
...
| 항목 | 비용 |
|------|------|
| OpenAI GPT-4o-mini | ~$0.50/월 (하루 9건 기준) |
| 서버 (VPS) | $5-10/월 |
| Ghost / WordPress / PostgreSQL | $0 (셀프호스팅) |
| **합계** | **~$6-11/월** |
